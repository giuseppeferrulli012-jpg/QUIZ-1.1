# quiz_calcio_unico.py

import streamlit as st
import random
# Pandas non è più strettamente necessario, ma lo manteniamo per abitudine in analisi dati
# import pandas as pd 

# --- DATI DEL QUIZ: 50 DOMANDE (Inserisci qui le tue 50 domande!) ---
...
# Usa il numero effettivo di domande disponibili, o un massimo di 10 se ne hai meno di 10
def initialize_quiz_state():
    """Imposta o resetta lo stato della sessione."""
    
    # Mescola TUTTE le domande e ne prende un sottoinsieme (es. 10 per una sessione rapida)
    # Se vuoi usare TUTTE le domande disponibili, imposta num_questions_to_use = len(ALL_QUIZ_QUESTIONS)
    NUM_DOMANDE_DA_USARE = 10 
    
    # Se ci sono meno di 10 domande, usiamo tutte quelle disponibili
    if len(ALL_QUIZ_QUESTIONS) < NUM_DOMANDE_DA_USARE:
         NUM_DOMANDE_DA_USARE = len(ALL_QUIZ_QUESTIONS)
         
    # Seleziona le domande casuali
    ...
    st.session_state['current_question_key'] = random.randint(0, 1000000) # Chiave unica per resettare i radio button
...
def check_answer(user_answer, correct_answer):
    ...
    # Rerun per visualizzare il feedback immediatamente
...
def next_question():
    ...
    st.session_state['feedback_message'] = ""
    st.session_state['current_question_key'] = random.randint(0, 1000000) # Resetta la chiave
...
def main():
    ...
    # Inizializzazione o Reset dello Stato
    if 'quiz_finished' not in st.session_state:
        initialize_quiz_state()
    
    # Numero di domande da usare
    ...
    # Recupera le opzioni e le mescola
    ...
    user_choice = st.radio(
        ...
        options,
        index=None,  # Nessuna opzione selezionata di default
        key=st.session_state['current_question_key'] # Resetta il radio button ad ogni domanda
    )
    ...
    with col1:
        # Se l'utente ha selezionato una risposta ma non ha ancora ricevuto feedback
        ...
        # Se c'è un feedback, mostra il pulsante "Avanti"
